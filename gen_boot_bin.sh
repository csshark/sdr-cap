...
if [[ "$HDF_FILE" =~ ".hdf" ]];then
	...
	SYSTEM_TOP_BIT_PATH="$BUILD_DIR/build/sdk/hw_0/sdr-cap.bit"
else
	echo 'platform create -name hw0 -hw system.xsa -os standalone -out ./build/sdk -proc $cpu_name' >> $BUILD_DIR/create_fsbl_project.tcl
	...
	SYSTEM_TOP_BIT_PATH="$BUILD_DIR/build/sdk/hw0/hw/sdr-cap.bit"
fi
...
echo 'sdr-cap.bit' >> $OUTPUT_DIR/zynq.bif
...
cp $SYSTEM_TOP_BIT_PATH $OUTPUT_DIR/sdr-cap.bit
